...
import numpy as np
import random
from data_loader_miid_all_selected import load_data
import os
import math
import torch.nn.functional as F
from collections import OrderedDict
import torchvision.models as models
from model.cmf_net_5 import CMFNET,get_Initial
...
save_interval = 10
best_loss = 1e6
model_path = '../model_output/MIID_multi_5_train_all_select_lr_3e-4/'
...
def mean_square_loss(pred_illum, target_illum, pred_respo, target_respo):
    ...
    return (mse_L + 0.2*mse_R + 0.001*torch.log(s.unsqueeze(-1))).mean()
...
def angular_loss(pred, target):#35.1754
    ...
    # 计算角误差
    loss = torch.acos(torch.clamp(torch.sum(pred * target, dim=1), min=-1.0, max=1.0))
...
res_dir = '../prec_illum/MIID_multi_5_train_all_select_lr_3e-4/epoch_0150/'
...
dataloader_train, dataloader_val = load_data(batch_size, num_workers, args.set)
...
with torch.no_grad():
    ...
    for batch_inputs, batch_gt_L, batch_gt_R, batch_names in dataloader_val:
        ...
        batch_inputs, batch_gt_L, batch_gt_R = batch_inputs.cuda(device=device_ids[0]), \
            batch_gt_L.cuda(device=device_ids[0]), batch_gt_R.cuda(device=device_ids[0])  # 0multi

        # 前向传播...
        # batch_output_L, batch_output_R = model.forward(batch_gt_L, batch_inputs)
        batch_output_L, batch_output_R = model.forward(batch_inputs, batch_inputs) # test
        # save mat
        i = 0
        for pred in batch_output_L:
            # pred = batch_output_L[i]
...
print("Val ae: {:.4f}, Val mse: {:.4f}, Val abe: {:.4f}"
      .format(val_loss_ae, val_loss_mse, val_loss_abe))
