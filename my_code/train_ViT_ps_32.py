...
val_interval = 10
best_loss = 1e6
model_path = '../model_output/ViT_ps32/'
...
class SpectrumEstimationViT(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, channel, dim, depth, heads, mlp_dim):
        ...
        patch_dim = dim  # Assuming input is a -channel image
        self.dim = dim
        ...
        # self.fc = nn.Linear(dim, channel)
        for module in self.modules():
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                nn.init.xavier_uniform_(module.weight)
    def forward(self, x):
        zero_tensor = torch.zeros(x.shape[0], self.dim-x.shape[1], x.shape[2], x.shape[3])
        zero_tensor=zero_tensor.cuda(device=x.get_device())
...
def print_angular_loss(pred, target):#35.1754
    ...
    print('pred_norm', pred_norm[0])
    print('target_norm',target_norm[0])
    # 计算角误差
...
for epoch in range(max_epochs):
    ...
    for batch_inputs, batch_targets, _, batch_names in dataloader_train:
    ...
    if (epoch + 1) % val_interval == 0 or epoch < val_interval:
        ...
        with torch.no_grad():
            ...
            iter_i = 0
            num_iter = len(dataloader_train)
            for val_inputs, val_targets, _, batch_names in dataloader_val:
                if (iter_i * batch_size) % 100 == 0:
                    print("iter [{}/{}],".format(iter_i * batch_size, num_iter * batch_size))
                    # print(count_parameters(model))
                iter_i += 1
