...
val_interval = 10
best_loss = 1e6
model_path = '../model_output/PWIR_v1_s1/'
...
for epoch in range(max_epochs):
    ...
    for batch_inputs, batch_targets, batch_names in dataloader_train:
        ...
        patches = crop_into_patches(batch_inputs, patch_size)
        for patch_i, patch in enumerate(patches, start=1):
            # if patch_i == 1 and epoch == 0 and iter_i == 0:
            #     draw_triple_Gaussian(patch)
            patch = patch.cuda(device=device_ids[0])
            patch_x3 = triple_Gaussian(patch)
            patch_x3, batch_targets = patch_x3.cuda(device=device_ids[0]), batch_targets.cuda(device=device_ids[0])
            # 前向传播
    ...
    if (epoch + 1) % val_interval == 0 or epoch < val_interval:
        ...
        print(patch_i)
        train_loss_ae /= len(dataloader_train)*patch_i
        train_loss_abe /= len(dataloader_train)*patch_i
