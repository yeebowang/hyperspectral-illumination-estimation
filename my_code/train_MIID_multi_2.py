...
for epoch in range(40, max_epochs):
    # dataloader_train, dataloader_val = load_data(batch_size, num_workers)
    # model = model.cpu()
    # 训练
    model.train()
    ...
    if (epoch + 1) % save_interval == 0 or epoch == 0:
        ...
        print("Epoch [{}/{}], Train ae: {:.4f}, Train mse: {:.4f}, Val ae: {:.4f}, Val mse: {:.4f}"
              .format(epoch + 1, max_epochs, train_loss_ae, train_loss_mse, val_loss_ae, val_loss_mse))
        ...
        train_loss_mse /= len(dataloader_train)
        train_loss_ae /= len(dataloader_train)
        print("Epoch [{}/{}], Train ae: {:.4f}, Train mse: {:.4f}"
              .format(epoch + 1, max_epochs, train_loss_ae, train_loss_mse))
