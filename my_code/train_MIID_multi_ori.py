...
from model.cmf_net_ori_simple import CMFNET#,get_Initial
...
save_interval = 5
best_loss = 1e6
model_path = '../model_output/MIID_multi_ori/'
...
for epoch in range(start_epoch,max_epochs):
    ...
    if 1:
        for batch_inputs, batch_gt_L, batch_gt_R, batch_names in dataloader_train:
            ...
            batch_inputs, batch_gt_L, batch_gt_R = \
                batch_inputs.cuda(device=device_ids[0]), \
                    batch_gt_L.cuda(device=device_ids[0]), \
                    batch_gt_R.cuda(device=device_ids[0])  # , \
            # batch_inital.cuda(device=device_ids[0]) # 0multi
